...
# Load the model
model_path = r'C:\Users\shara\Downloads\FoodDetectionandCalorieEstimation-main\FoodDetectionandCalorieEstimation-main\saved_models\my_model.keras'
model = load_model(model_path)
print("Model loaded Successfully!")

# Load the nutritional dataset
nutrition_file_path = r"C:\Users\shara\Downloads\FoodDetectionandCalorieEstimation-main\FoodDetectionandCalorieEstimation-main\food20dataset\metadata\dataset.csv"
...
def get_nutritional_info(food_name):
    # Ensure 'Food Item' column has no NaN values
    nutrition_df.dropna(subset=['Food Item'], inplace=True)
    
    # Normalize spaces and case in the dataset for comparison
    nutrition_df['Food Item'] = nutrition_df['Food Item'].str.lower().str.strip().str.replace(r'\s+', ' ', regex=True)
    
    # Normalize the input food name
    food_name = food_name.lower().strip()

    # Find matching food item
    food_info = nutrition_df[nutrition_df['Food Item'] == food_name]
        ...
        def parse_value(value):
            try:
                if pd.isna(value) or value in ["", "N/A"]:
                    return "N/A"  # Keep as N/A instead of defaulting to 0
                
                str_value = str(value).replace('g', '').strip()
                
                # Handle range values like "10-15" or "10 – 15"
...
# Rest of the existing routes (predict, uploads, index, intake, logout) remain the same
